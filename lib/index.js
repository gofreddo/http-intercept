...
module.exports = (options) => {
  if (!_.isObject(options)) throw new Error('options should be an object;')
  const iterceptOptions = Object.assign({}, options);

  const searchAndReplaceValues = [];
  if (!_.isNil(iterceptOptions.searchAndReplaceValues) &&
    _.isArray(options.searchAndReplaceValues)) {
    searchAndReplaceValues.push.apply(searchAndReplaceValues, iterceptOptions.searchAndReplaceValues);
  }
  ...
  http.request = (options, callback) => {
    const newOpts = replaceOptions(options);
